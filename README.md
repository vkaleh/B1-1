...
PID=$(pgrep -f "$APP_NAME" | head -1)
```
pgrep -f "$APP_NAME" : 프로세스 이름에 agent-app이 포함된 모든 PID를 한 줄에 하나씩 출력 <br>
head -1 : 여러 줄의 결과가 나오더라도 맨 위 1줄만 <br>
<br>
...
```
- command -v ss : ss 명령어가 시스템에 설치되어 있으면 해당 경로(예: /usr/sbin/ss)를 출력하고, 없으면 아무것도 출력하지 않으면서 실패 상태를 반환 <br>
- \>(or 1>) : 표준 출력(정상적인 결과물)의 방향을 화면이 아닌 다른 파일이나 장치로 돌리는(리다이렉션) 기호 <br>
- 2> : Error 메시지의 방향을 다른 파일이나 장치로 돌리는 기호. (숫자 2가 에러 통로를 의미) <br>
- /dev/null : 리눅스의 휴지통 같은 가상 장치. 이곳으로 보내진 모든 문자열 데이터는 흔적도 없이 사라짐 <br>
- &>/dev/null : &은 1번(정상)과 2번(에러) 통로 모두를 뜻함. 정상 출력이든 에러 메시지든 화면에 아무것도 보여주지 말고 전부 버리라는 뜻. (command -v ss가 성공하든 실패하든 화면을 깨끗하게 유지하기 위해 사용) <br>
- 2>/dev/null: 정상 출력은 그대로 두고, 혹시 에러 메시지가 발생하면 그것만 블랙홀로 버려라. netstat 명령어가 간혹 권한 문제 등으로 경고 에러를 뿜을 때, 스크립트 결과창이 지저분해지는 것을 막아줌 <br>
- :${APP_PORT}'([[:space:]]|$)' : 포트번호 뒤에 공백이나 줄이 끝이 와야 함
- netstat -tulnp : TCP 포트, UDP 포트, 열려있는 (Listening) 포트만 보여주고, 숫자(Numeric/ 127.0.0.1이나 80 같은) 형태로, 그 포트를 열고 있는 프로세스 이름과 PID(Program)을 같이 출력해줘 
<br>
...
- 크기 검사 : stat -c%s 명령어로 monitor.log의 용량을 바이트 단위로 체크.
- 번호 밀어내기 : 10 MB를 넘어가면 기존 로그들의 파일 번호를 하나씩 뒤로 밀어냄. 7->8, 8->9, ...
- 현재 로그 백업 및 초기화 : 10 MB가 넘었던 monitor.log를 비워둔 monitor.log.1로 백업. 그 후 새로운 monitor.log를 만듦
<br>

### 6-4. 경고는 출력하되 종료하지 않는 항목(방화벽 비활성/임계치 초과)을 분리한 운영상의 이유가 뭘까?
방화벽이 꺼졌거나 용량이 좀 찼다고 해서 서버 프로그램을 아예 종료시켜 버리면 모든 사용자가 서비스에 접속하지 못하게 됨 <br>
그래서 데이터가 깨지거나, 더 이상 작동할 수 없는 치명적인 에러일 때만 프로그램을 종료하고, <br>
보통 경고만 해서 서비스는 살려두고 그 사이에 운영자가 방화벽을 다시 키거나 디스크를 비우는 조치를 함 <br>
<br>

### 6-5. 리다이렉션 기호 >와 >>의 차이, 로그 누적에 >>가 필요한 이유?
\>는 덮어쓰기(기존 내용 싹 지움), >>는 이어쓰기(뒤에 덧붙임) <br>
로그는 과거 기록들을 누적시켜야 하므로 >>를 사용 <br>
<br>

### 6-6. 모니터링 대상이 웹 서버(Nginx 등)로 바뀐다면, monitor.sh에서 바꿔야 할 핵심 포인트?
monitor.sh가 Nginx를 감시하기 위해 프로세스 이름을 agent-app에서 Nginx로, <br>
포트 번호를 Nginx의 포트 번호로 수정해야 함 <br>
<br>

### 6-7. 프로세스는 살아있는데 포트가 안 열리는 상황을 발견했다면, 원인 후보와 확인 순서는 뭘까?
원인 후보
- 대기(Listen) 상태 오류: 프로그램이 켜지긴 했는데, 어떤 포트로 손님을 맞이할지 내부 설정 오류로 헤매고 있는 경우
- 포트 충돌: 이미 다른 프로그램이 해당 포트를 선점해서 쓰고 있는 경우
- 방화벽 차단: 프로그램은 포트를 열었지만, 리눅스 자체 방화벽이 그 포트로 들어오는 길을 막고 있는 경우

확인 순서
- 1단계 (netstat 또는 ss 명령어): 프로그램이 진짜 포트를 열고 기다리는 중인지 시스템 내부를 먼저 확인
- 2단계 (로그 파일 확인): 로그를 열어 포트를 열다가 에러가 났는지 확인(예: Address already in use 같은 문구가 있는지)
- 3단계 (방화벽 확인): ufw나 iptables 명령어로 해당 포트가 외부와 통신할 수 있게 허용되어 있는지 확인
<br>

### 6-8. 로그가 급증해 디스크가 가득 찰 위험이 있다면, 운영자가 취할 대응은?
응급 조치
- 오래된 로그 압축 및 삭제: .log.gz로 끝나는 옛날 압축 파일이나 몇 달 전 로그 파일을 지우거나, 다른 외부 저장소(백업 서버)로 이동시켜 빈 공간을 확보
- 현재 쌓이고 있는 로그 파일 비우기: 서비스를 끄지 않고 용량만 0으로 만드는 명령(cp /dev/null app.log)을 내리기

근본 해결 (재발 방지)
- 로그 레벨 조절: 프로그램이 너무 사소한 것까지 로그에 쓰고 있다면(예: DEBUG 모드), 중요한 에러만 적도록 설정(INFO 또는 ERROR 모드)을 변경
- 자동 관리 설정: logrotate 주기를 더 짧게(예: 매달 -> 매일) 바꾸거나, 최대 보관 용량 제한을 타이트하게 잡기

<img width="2184" height="1093" alt="Screenshot 2026-05-23 at 1 28 22 PM" src="https://github.com/user-attachments/assets/3d69e23e-9a4a-4767-80f7-6cd0fd9e96f0" />
